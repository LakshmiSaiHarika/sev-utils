...
cloud_init_create_data() {
  if [[ -f "${LAUNCH_WORKING_DIR}/${GUEST_NAME}-data/meta-data.yaml" && \
...
# Seed image metadata
cat > "${LAUNCH_WORKING_DIR}/${GUEST_NAME}-data/meta-data.yaml" <<EOF
...
EOF
  ...
  cloud-localds "${LAUNCH_WORKING_DIR}/${GUEST_NAME}-seed.img" \
    ...
    "${LAUNCH_WORKING_DIR}/${GUEST_NAME}-data/meta-data.yaml"
