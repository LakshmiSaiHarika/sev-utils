...
download_guest_os_image(){
  ...
  case ${linux_distro} in
    ubuntu)
      ...
      ;;
    fedora)
      CLOUD_INIT_IMAGE_URL="https://archives.fedoraproject.org/pub/archive/fedora/linux/releases/37/Cloud/x86_64/images/Fedora-Cloud-Base-37-1.7.x86_64.qcow2"
      ;;
  esac
...
get_guest_kernel_package(){
  ...
  pushd "${SETUP_WORKING_DIR}/AMDSEV/linux" >/dev/null
    case ${linux_distro} in
      ubuntu)
          echo $(realpath linux-image*snp-guest**${guest_kernel_version}*.deb| grep -v dbg)
          ;;
      fedora)
